...
#create counter variable which will be used for concatenating substrings into patterns
...
#identify alternative-wise and attribute-wise patterns ####

#create alternative-wise and attribute-wise substrings based on counter variable
...
#write function that identifies rows with identical elements only and apply it
...
#write function that keeps unique elements and sorts them alphabetically and apply it to alternative-wise substrings
...
substrings = anti_join(substrings, altwiseSubstrings, by = c("counter", "participant", "condition", "trial", "identElements")) %>% bind_rows(altwiseSubstrings)
...
patternsSim = function() { #function which contains random version of data
  ...
  #identify alternative-wise and attribute-wise transitions 
  n = nrow(infoSearch)
  infoSearch$transAlt = c(0L, infoSearch$trial[-n] == infoSearch$trial[-1] & infoSearch$alternative[-n] == infoSearch$alternative[-1])
  infoSearch$transAtt = c(0L, infoSearch$trial[-n] == infoSearch$trial[-1] & infoSearch$attribute[-n] == infoSearch$attribute[-1])
