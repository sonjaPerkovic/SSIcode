...
#create variable that assigns 1 to equal subsequent counter variable values
altwiseStrings$equalCounter <- ifelse(altwiseStrings$counter == lag(altwiseStrings$counter, n = 1L)
                                      | altwiseStrings$counter == lead(altwiseStrings$counter, n = 1L), 1, 0)
...
altwiseSim <- function() { #function which contains random version of data
  ...
  #import data (same file as in line 11)
  infoSearch <- as.data.table(read.csv("fileName.csv", header = T, sep = ";"))
  ...
  num_att <- 4 #number of attributes in the experiment
  attset <- letters[1:num_att]
  infoSearch$attribute <- sample(attset, sim, T) #sample first four letters in the alphabet (a, b, c and d)
  #infoSearch$attribute <- sample(c("b", "f", "p", "s"), sim, T) #use this line if you want to test gaborClean data set
  #in addition, lines 152-154 need to be commented out
...
altwiseSimRep <- do.call(rbind, mclapply(1:10000, function(i) altwiseSim()))
...
attwiseSim <- function() { #function which contains random version of data
  ...
  infoSearch$attribute <- sample(attset, sim, T) #sample first four letters in the alphabet (a, b, c and d)
  #infoSearch$attribute <- sample(c("b", "f", "p", "s"), sim, T) #use this line if you want to test gaborClean data set
  #in addition, lines 299-301 need to be commented out
...
attwiseSimRep <- do.call(rbind, mclapply(1:10000, function(i) attwiseSim()))
...
#merge in total string length per participant per trial #calculated in line 58
...
#merge in total string length per participant per trial #calculated in the line 58
