...
substrings = anti_join(substrings, altwiseSubstrings, by = "counter") %>%
...
#set the threshold for pattern length
threshold = 4

if (threshold == 4) {

#create variable that assigns 1 to subsequent equal counter variable values
substrings$equalCounter = ifelse(substrings$countEqualSubstrings == shift(substrings$countEqualSubstrings, 1L) |
                                 substrings$countEqualSubstrings == shift(substrings$countEqualSubstrings, 1L, type = "lead"), 1, 0)
...
patternsCount$probComplement = 1 - patternsCount$probability
